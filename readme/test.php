...
switch($_GET['a'])
{
	case 'index':
			...
			<a href="index.php?m=test&a=goods" target="_blank">积分商品测试</a>
			';
		break;
	case 'shop':
		for($i=0;$i<100;$i++)
	...
	case 'art':
		for($j=0;$j<10;$j++)
		{
			$db->query("INSERT INTO ".table('art_cat')." SET cname='test".time());
			$catid=$db->insert_id();
			for($i=0;$i<100;$i++)
			{
				$db->query("INSERT INTO ".table('art')." SET siteid=1,catid='$catid',userid=400,dateline=".time().",title='测试".time()."' ");
			}
		}
