...
function onAdd_db()
{
	...
	if(!$ct)
	{
		...
		$GLOBALS['db']->query("INSERT INTO ".table('shopcar')." SET userid='$userid',ssid='{$GLOBALS['ss_id']}',caiid='$caiid',cainum='$cainum',shopid=".$cai['shopid']." ,title='".$cai['title']."',price='$price' ");
	}else
	{
...
{	
	...
	//删除购物车
	$db->query("delete from ".table('shopcar')." where userid='$userid' or ssid='".$_GLOBAL['ss_id']."' ");
	...
	{	
		...
		if((PHONE_ON==1) or $shopconfig['isemail'])
		{
			...
			{		
				...
				//发送给商家
				$phone=$db->getOne("SELECT phone FROM ".table('shop')." WHERE shopid=".$order['shopid']."  ");
				$res = sendSMS(PHONE_USER,PHONE_PWD,$phone,$content);		
				//发送给用户			
