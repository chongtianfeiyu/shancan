...
if($a=='index')
{
	...
	while($rs=@$db->fetch_array($res))
	{
		if($rs['status']==0)
		{
			$rs['dotype']="<a href=\"shopadmin.php?m=user&a=dotype&userid=".$rs['userid']."&status=1\">未审核</a>";
		}elseif($rs['status']==1)
		{
			$rs['dotype']=" <a href=\"shopadmin.php?m=user&a=dotype&userid=".$rs['userid']."&status=-1\">已审核</a>";
		}elseif($rs['status']==-1)
		{
			$rs['dotype']=" <a href=\"shopadmin.php?m=user&a=dotype&userid=".$rs['userid']."&status=1\">已禁止</a>";
		}
