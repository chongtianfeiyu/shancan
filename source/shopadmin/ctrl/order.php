...
{
	$w="";
	$sql="select  *  from ".table('order')." where shopid='$shopid'  AND isvalid=1  ";
	$sql2="select count(*) from ".table('order')."  where shopid='$shopid'  AND isvalid=1  ";
	$sql3=" select SUM(money) as summoney, AVG(money) as avgmoney from ".table('order')." where shopid='$shopid'  AND isvalid=1  ";
	$orderno=$_GET['orderno'];
	$uid=intval($_GET['uid']);
	$url="shopadmin.php?m=order&";
	...
	//分页结束
	$ids=$db->getCols($sql);
	$res=$db->query("SELECT * FROM ".table('order')." WHERE id in("._implode($ids).") ORDER BY id DESC ");
	$orderlist=array();
...
{
	$id=intval($_GET['id']);
	$rs=$db->getRow("select o.*,u.username,u.phone,u.address from ".table('order')." as o left join ".table('user')." as u on o.userid=u.userid  where o.id='$id' and o.shopid='$shopid' ");
...
{
	$id=intval($_GET['id']);
	$db->query("update ".table('order')." set ctype=1 where id='$id' and shopid='$shopid' ");
	gourl();
}elseif($a=='del')
{
/*	$id=intval($_GET['id']);
	$db->query("delete from ".table('order')." where id='$id'  and shopid='$shopid' ");
	//删除订单详情
	$db->query("delete from ".table('order_cai')." where orderid='$id'  and shopid='$shopid' ");
	gourl();*/
}elseif($a=='sendtype')
{
	...
	if(is_array($_POST['id']))
	{
		...
		if($sendtype==3)
		{
			...
			foreach($rs as $key=>$val)
			{
				...
				//实际支付的费用
				$money=(GRADE_ON==1)?$val['money']*$discount/100:$val['money'];
				if($val['ispay']==1)
				{
	...
	if($_GET['orderid'])
	{
		...
		if($sendtype==3)
		{
			...
			if($rs)
			{
				...
				//实际支付的费用
				$money=(GRADE_ON==1)?$rs['money']*$discount/100:$rs['money'];
				if($rs['ispay']==1)
				{
...
}elseif($a=='senddes')
{
	...
	$db->query("update ".table('order')." set senddes='$senddes' where id='$id'  and shopid='$shopid' ");	
	gourl();
}elseif($a=='tocsv')
{
	$w=" AND isvalid=1   ";
	...
	$sql="select id,orderno,money,orderuser ,orderphone,orderaddress,dateline from ".table('order')." where   shopid='$shopid'   $w  ";
	$arr[]=array("订单编号 ","价格 ",'联系人  ','联系电话  ','联系地址  ',"时间","所点菜");
	...
	while($rs=$db->fetch_array($res))
	{
		...
		$rs['cai']=implode("，",$db->getCols("select title from ".table('cai')." where id in("._implode($cid).") "));
		
		$arr[]=array($rs['orderno'],$rs['money'],$rs['orderuser'],$rs['orderphone'],$rs['orderaddress'],$rs['dateline'],$rs['cai']);
	}
